  ...
  return (
    <Card sx={{p:2, maxWidth:'md', mx:'auto'}}>
        <Box display='flex' justifyContent='space-between' alignItems='center'>
            ...
            <Button component={Link} to='/orders' variant='outlined'>Back to orders</Button>
        </Box>

        <Divider sx={{my:2}}/>

        <Box>
            <Typography variant="h6" fontWeight='bold'>Billing and Deliver information</Typography>
            <Box component='dl'>
                ...
                <TableBody>
                    {order.orderItems.map((item) => (
                        <TableRow key={item.productId} sx={{borderBottom:'1px solid rgba(224, 224, 224,1)'}}>
                            ...
                            <TableCell align="right" sx={{p:4}}>{currencyFormat(item.price * item.quantity)}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
        ...
        <Box sx={{mx: 3, mt: 2}}>
            ...
            <Box component='dl' display='flex' justifyContent='space-between'>
                ...
                <Typography component='dd' variant="body2" fontWeight='300' color='green'>{currencyFormat(order.discount)}</Typography>
            </Box>
            <Box component='dl' display='flex' justifyContent='space-between'>
